(function () {
    ...
    function ImportController ($scope, $q, gettext, api) {
        ...
        async function checkImportFile (importFile) {
            const importBundle = {
                layers: [],
                reports: [],
                dashboards: [],
            ...
            for (const layer of importFile.layers) {
                ...
                try {
                    const l = await api.getLayer(layer._id);
                    if (l) {
            ...
            for (const report of importFile.reports) {
                ...
                try {
                    const r = await api.getReport(report._id);
                    if (r) {
            ...
            for (const dashboard of importFile.dashboards) {
                ...
                try {
                    const d = await api.getDashboard(dashboard._id);
                    if (d) {
